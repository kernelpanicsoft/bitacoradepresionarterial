...
interface TomaDAO {
    ...
    @Query("SELECT * FROM Toma WHERE Toma.usuario_id = :id ORDER BY datetime(Toma.fecha_hora) DESC")
    fun getTomasUsuarioDesc(id: Int) : LiveData<List<Toma>>

    @Query("SELECT * FROM Toma WHERE Toma.usuario_id = :id ORDER BY datetime(Toma.fecha_hora) ASC")
    ...
    @Query("SELECT * FROM Toma WHERE TOma.usuario_id = :id ORDER BY CASE WHEN :order = 1 THEN datetime(Toma.fecha_hora) END ASC, CASE WHEN :order = 0 THEN datetime(Toma.fecha_hora) END DESC")
    fun getTomasUsuarioOrdendas(id: Int, order: Int) : LiveData<List<Toma>>

    @Query("SELECT * FROM Toma WHERE Toma.usuario_id = :id AND Toma.valoracion = :valoracion ORDER BY CASE WHEN :order = 1 THEN datetime(Toma.fecha_hora) END ASC, CASE WHEN :order = 0 THEN datetime(Toma.fecha_hora) END DESC")
    fun getTomasUsuarioPorValoracion(id: Int, valoracion: Int, order: Int) : LiveData<List<Toma>>

    @Query("SELECT * FROM Toma WHERE Toma.usuario_id = :id AND Toma.momento = :momento ORDER BY CASE WHEN :order = 1 THEN datetime(Toma.fecha_hora) END ASC, CASE WHEN :order = 0 THEN datetime(Toma.fecha_hora) END DESC")
    fun getTomasUsuarioPorMomento(id: Int, momento: Int, order: Int) : LiveData<List<Toma>>

    @Query("SELECT * FROM Toma WHERE Toma.usuario_id = :id AND Toma.extremidad = :extremidad ORDER BY CASE WHEN :order = 1 THEN datetime(Toma.fecha_hora) END ASC, CASE WHEN :order = 0 THEN datetime(Toma.fecha_hora) END DESC")
    fun getTomasUsuarioPorExtremidad(id: Int, extremidad: Int, order: Int) : LiveData<List<Toma>>

    @Query("SELECT * FROM Toma WHERE Toma.usuario_id = :id AND Toma.posicion = :posicion ORDER BY CASE WHEN :order = 1 THEN datetime(Toma.fecha_hora) END ASC, CASE WHEN :order = 0 THEN datetime(Toma.fecha_hora) END DESC")
